...
- [x] Tier 3 Part B: Kubernetes Validating Admission Webhook (Live `kind` cluster `kubectl apply` verification — **not re-confirmed as of commit `34f9df9` / 2026-09-04; no cluster available at that point**)
  > ⚠️ *Caveat (added commit `34f9df9`, 2026-09-04):* Live-cluster evidence is from a prior session with no recoverable git trail. Until a cluster is re-provisioned and the full `kubectl apply` + rejection sequence is re-run and committed as evidence, treat this as "last-known-good." A future session that re-confirms should note "supersedes `34f9df9`" in its commit message.
