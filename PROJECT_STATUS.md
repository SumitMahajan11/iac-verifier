...
- [x] Tier 3 Part B: Kubernetes Validating Admission Webhook (Live `kind` cluster `kubectl apply` verification — verified live in a prior session; **not re-confirmed in current session, no cluster available**)
  > ⚠️ *Caveat (added 2026-09-04):* Live-cluster evidence is from a previous conversation. Until a cluster is re-provisioned and the full `kubectl apply` sequence is re-run, treat this as "last-known-good" rather than currently verified.
